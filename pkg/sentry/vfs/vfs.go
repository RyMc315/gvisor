...
func (vfs *VirtualFilesystem) AccessAt(ctx context.Context, creds *auth.Credentials, ats AccessTypes, pop *PathOperation) error {
	...
	for {
		vfs.blockOnMountPromise(ctx, rp)
...
func (vfs *VirtualFilesystem) RegisterMountPromise(vd VirtualDentry) {
	vfs.mountPromisesMu.Lock()
	defer vfs.mountPromisesMu.Unlock()
	if _, ok := vfs.mountPromises[vd]; ok {
		log.Warningf("mount promise for %v already exists", vd)
		return
	}
	wq := &waiter.Queue{}
...
func (vfs *VirtualFilesystem) blockOnMountPromise(ctx context.Context, rp *ResolvingPath) {
...
func (vfs *VirtualFilesystem) resolveMountPromise(vd VirtualDentry) {
	vfs.mountPromisesMu.Lock()
	defer vfs.mountPromisesMu.Unlock()
	wq, ok := vfs.mountPromises[vd]
	if !ok {
